...
BEFORE YOU RUN IT:
  ...
  * Near a mechanical stop, use SMALL steps and do not park the servo stalled
    ...
    metal gears transmit that to whatever stops it.
...
def main():
    ...
    if not PWM_MIN_US <= args.start <= PWM_MAX_US:
        sys.exit(f"--start {args.start} is outside the {PWM_MIN_US}-"
                 f"{PWM_MAX_US}us guard")
    ...
    def goto(us, why):
        ...
        nonlocal pos
        if not PWM_MIN_US <= us <= PWM_MAX_US:
            print(f"  REFUSED: {us}us is outside the {PWM_MIN_US}-{PWM_MAX_US}us "
                  f"guard. That guard is here to catch a typo before it reaches "
                  f"the servo, so widen it in mavlink_io.py deliberately or not "
                  f"at all.")
            return False
